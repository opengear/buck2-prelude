...
RustDoctestSourceInfo = provider(
    ...
    fields = {
        ...
        # SVH.
        "crate_rlib": Artifact,
        # crate_extern_args - the library's own `--extern=<name>=<lib>` in the
        # prelude's extern wire format (dynamic crate names included), the
        # identical binding the persist compile built the generated bundle
        # against. A consumer compiling generated doctest source splices it in
        # without re-deriving the crate name; the crate name lives on the
        # `rust_library` definition and this is its one doctest-facing
        # encoding.
        "crate_extern_args": cmd_args,
...
RustdocTestAssembly = record(
    ...
    # The library's own `--extern=<name>=<lib>` in the prelude's extern wire
    # format (`extern_arg`, dynamic crate names included). The one binding
    # both the rustdoc compile and the persisted-source consumers use.
    crate_extern = field(cmd_args),
...
def rustdoc_test_assembly(
    ...
    # The library's own `--extern` binding, built once: rustdoc compiles the
    # doctests against it here, and the persist path surfaces the identical
    # binding on `RustDoctestSourceInfo` so a downstream compile of the
    # persisted source binds the library under the same crate name.
    crate_extern = extern_arg([], attr_crate(ctx), rlib)
...
def generate_rustdoc_persist(
    ...
    return RustDoctestSourceInfo(
        ...
        crate_extern_args = assembly.crate_extern,
        compile_context_args = cmd_args(
...
def rust_compile(
    ...
    # Ship a bin or cdylib crate with its debug separated out of band at link
    # time, so the full-debug artifact is never a declared output (no second
    # CAS copy). `separate_debug_info` is the per-target intent ("separate my
    # debug"); the mechanism it resolves to is derived from the toolchain by
    # `get_split_debug_container` -- a gnu-debuglink sidecar on ELF, a `.dSYM`
    # bundle on Mach-O, and none when a split mode already externalizes the
    # debug into `.dwo`/`.dwp`.
    #
    # A bin link's shipped output is predeclared, so that gate discriminates
    # it from the auxiliary `emit=link` compiles (remarks, profiling) that
    # share its `CrateType("bin")` but carry no predeclared output and must
    # not separate. A cdylib link is never predeclared by
    # `rust_library_impl`; no auxiliary `emit=link` compile in
    # `rust_library.bzl` uses cdylib params (e.g. `remarks_artifact` builds
    # against `meta_params`, a static/rlib-style build), so
    # `CrateType("cdylib")` alone discriminates the real cdylib link.
    want_separate_debug_bin = (
    ...
    want_separate_debug_cdylib = (
        requires_linking and
        params.crate_type == CrateType("cdylib") and
        getattr(ctx.attrs, "separate_debug_info", False)
    )
    want_separate_debug = want_separate_debug_bin or want_separate_debug_cdylib
    ...
    # The `.dSYM` sibling is declared next to the link output's own
    # `short_path`, a path proven deterministic for a predeclared bin output.
    # A cdylib link's output is never predeclared, so a cdylib on a Mach-O
    # toolchain keeps `split_debug_container` at `none` and ships whole.
    if want_separate_debug_cdylib and split_debug_container == SplitDebugContainer("dsym"):
        split_debug_container = SplitDebugContainer("none")
    ...
    if requires_linking:
        ...
        if deferred_link_enabled:
            ...
            # The ".debuginfo" suffix is a naming contract; tooling downstream
            # of the build identifies sidecars by it.
