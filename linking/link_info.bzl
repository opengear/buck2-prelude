...
LinkedObject = record(
    ...
    # A `.debuginfo` gnu-debuglink sidecar carrying the full DWARF, produced
    # inside the link action when the executable ships stripped with its debug
    # out of band. The `output` keeps a `.gnu_debuglink` reference to it.
